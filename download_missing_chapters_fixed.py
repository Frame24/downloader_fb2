...
"""
Исправленный скрипт для скачивания пропущенных глав
"""
...
def download_missing_chapters():
    ...
    # Пропущенные главы (только существующие)
    missing_chapters = [
    ...
    for chapter_num, volume in missing_chapters:
        ...
        try:
            ...
            print(f"   📊 Тип content: {type(data.get('content'))}")
            if "content" in data:
                content = data["content"]
                if isinstance(content, dict):
                    print(f"   📊 Content - словарь, ключи: {list(content.keys())}")
                    if "content" in content:
                        ...
                        print(f"   📊 Параграфов в JSON: {paragraph_count}")
                elif isinstance(content, str):
                    print(f"   📊 Content - строка, длина: {len(content)}")
                else:
            ...
            # Проверяем, что fb2_content - это строка
